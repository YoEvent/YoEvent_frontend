...
export default function ApplicationsPage() {
  ...
  const loadApplications = async (eventId: string) => {
    ...
    try {
      ...
      setSponsors((allSponsors || []).filter((s: any) => s.eventId === eventId || s.event?.eventId === eventId));
      setVolunteers((allNetworkings || []).filter((n: any) => (n.eventId === eventId || n.event?.eventId === eventId) && n.role === "VOLUNTEER"));
    ...
    <div className="flex bg-[#111] min-h-screen text-[#e0e0e0]">
      ...
      <div className="ml-[220px] flex-1 flex flex-col">
        <header className="h-[60px] bg-[#161616] border-b border-[#222] flex items-center justify-between px-8 sticky top-0 z-40">
          <h1 className="font-display text-xl font-bold text-white">Applications</h1>
          {events.length > 0 && (
            <div className="relative">
              <select
                value={selectedEventId}
                onChange={e => setSelectedEventId(e.target.value)}
                className="appearance-none bg-[#222] border border-[#333] text-[#ddd] text-sm rounded-lg px-4 py-1.5 pr-8 outline-none cursor-pointer"
              >
                {events.map(ev => (
        ...
        <main className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-5">
            <div className="bg-[#1e1e1e] border border-[#2a2a2a] rounded-2xl p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-white">{sponsors.length} <span className="text-sm font-normal text-amber-300">({pendingSponsors} pending)</span></p>
              </div>
            </div>
            <div className="bg-[#1e1e1e] border border-[#2a2a2a] rounded-2xl p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-white">{volunteers.length} <span className="text-sm font-normal text-blue-300">({pendingVolunteers} pending)</span></p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#1e1e1e] border border-[#2a2a2a] rounded-2xl p-6">
            <h2 className="font-display font-bold text-white mb-4">Acceptance Limits — {selectedEvent?.title || "Select an event"}</h2>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="w-24 bg-[#222] border border-[#333] text-[#ddd] rounded-lg px-3 py-1.5 text-sm outline-none focus:border-[#d4c9a8]"
                  />
                  <button onClick={() => saveLimit("maxSponsors", maxSponsors)} className="px-4 py-1.5 bg-[#d4c9a8] text-[#1a1a1a] text-xs font-bold rounded-lg hover:bg-[#c8bb96] transition-colors cursor-pointer">Save</button>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="w-24 bg-[#222] border border-[#333] text-[#ddd] rounded-lg px-3 py-1.5 text-sm outline-none focus:border-[#d4c9a8]"
                  />
                  <button onClick={() => saveLimit("maxVolunteers", maxVolunteers)} className="px-4 py-1.5 bg-[#d4c9a8] text-[#1a1a1a] text-xs font-bold rounded-lg hover:bg-[#c8bb96] transition-colors cursor-pointer">Save</button>
          ...
          <div className="flex gap-1 bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-1 w-fit">
            {([["sponsors", "Sponsors", Handshake], ["volunteers", "Volunteers", Users]] as const).map(([key, label, Icon]) => (
              <button key={key} onClick={() => setTab(key)}
                className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold transition-all cursor-pointer ${tab === key ? "bg-[#d4c9a8] text-[#1a1a1a]" : "text-[#666] hover:text-[#ccc]"}`}>
                <Icon size={15} />{label}
              </button>
            ))}
          </div>
          ...
          {loading ? (
            <div className="flex justify-center py-16"><div className="w-8 h-8 border-4 border-[#333] border-t-[#d4c9a8] rounded-full animate-spin" /></div>
          ) : tab === "sponsors" ? (
            <div className="space-y-3">
              {sponsors.length === 0 ? (
                <div className="bg-[#1e1e1e] border border-[#2a2a2a] rounded-2xl p-10 text-center text-[#555]">No sponsor applications for this event.</div>
              ) : sponsors.map((s) => {
                const id = s.sponsorId || s.id;
                return (
                  <div key={id} className="bg-[#1e1e1e] border border-[#2a2a2a] rounded-2xl p-5 flex items-center gap-5">
                    {/* Logo */}
                    <div className="w-14 h-14 rounded-xl bg-[#252525] border border-[#333] flex items-center justify-center shrink-0 overflow-hidden">
                      {s.logo || s.logoUrl ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-white truncate">{s.companyName || "Unknown Company"}</p>
                    ...
                    {s.sponsorshipAmount && (
                      <div className="text-right shrink-0">
                        ...
                        <p className="text-sm font-bold text-[#d4c9a8]">{Number(s.sponsorshipAmount).toLocaleString()} FCFA</p>
                      </div>
                    )}
                    {/* Status */}
            ...
            <div className="space-y-3">
              {volunteers.length === 0 ? (
                <div className="bg-[#1e1e1e] border border-[#2a2a2a] rounded-2xl p-10 text-center text-[#555]">No volunteer applications for this event.</div>
              ) : volunteers.map((v) => {
                const id = v.connectionId || v.id;
                return (
                  <div key={id} className="bg-[#1e1e1e] border border-[#2a2a2a] rounded-2xl p-5 flex items-center gap-5">
                    {/* Photo */}
                    <div className="w-14 h-14 rounded-full bg-[#252525] border border-[#333] flex items-center justify-center shrink-0 overflow-hidden">
                      {v.photoUrl ? (
                    ...
                    {/* Info */}
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-white truncate">{v.name || `Volunteer ${(id || "").substring(0, 8)}`}</p>
